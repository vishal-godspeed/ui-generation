...
function renderTickerMD() {
  const bar = document.getElementById('stock-ticker-bar-md');
  bar.innerHTML = stocksMD.map(stock => `
    <div class="flex items-center space-x-2 min-w-max">
      ${getIconMD(stock.color)}
...
function updatePricesMD() {
  stocksMD.forEach(stock => {
...
renderTickerMD();
setInterval(updatePricesMD, 2000);
autoScrollTickerMD();
